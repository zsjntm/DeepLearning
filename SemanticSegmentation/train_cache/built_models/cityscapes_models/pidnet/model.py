...
class PIDNet(nn.Module):
    ...
    def __init__(self, m=2, n=3, num_classes=19, planes=64, ppm_planes=96, head_planes=128, augment=True):
        ...
        if m == 2:
            ...
            self.spp = PAPPM(1024, ppm_planes, 256)
        ...
        # 前四个stage和stem换成了resnet18
        self.conv1 = nn.Sequential(*list(r18.children())[:4])
    ...
    def forward(self, x):
        ...
        x = self.conv1(x)
        x = self.layer1(x)
        x = self.relu(self.layer2(self.relu(x)))  # s2激活
        x_ = self.layer3_(x)
        x_d = self.layer3_d(x)

        x = self.relu(self.layer3(x))  # s3激活
        ...
        if self.augment:
            ...
            # return x_
            return F.interpolate(x_, x_size, mode='bilinear')  # 将输出upsample到原尺寸
...
def build_model():
    return PIDNet(2, 2, 19, 64, 96, 128, False)
...
if __name__ == '__main__':
    ...
    # model = get_pred_model(name='pidnet_m', num_classes=21)
    # model.eval()
    # model.to(device)
    # iterations = None
    #
    ...
    model = build_model()  # 1.I分支 -> r18; 2.输出bilinear上采样; 3.通用训练配方
    print(model)
